...
inputs = {"markers": markers.copy(), "nirs": nirs.copy(), "drone": drone.copy(),
          "traits_covs": traits_df, "weather": OD["weather_smms"].copy()}
...
traits_df_r = inputs["traits_covs"].loc[pos_notna]
...
x_trait_df = traits_df_r[traits_covs]
...
pred_dict = {"genotype": XR_f(x_markers.to_numpy()), "NIRs": XR_f(x_nirs.to_numpy()),
             ...
             "Weather": x_weather.to_numpy()}

dir_folds = os.path.join(dir_outs0, "../../folds_df_variety-CV1.csv")
...
for ok in range(1, K + 1):
    ...
    dir_outs = os.path.join(dir_outs0, "Preds_" + predictor_label + "-Trait_" + trait + ".csv")
    save_csv(df_Preds, dir_outs)
    dir_outs = os.path.join(dir_outs0, "Tab-Smm_" + predictor_label + "-Trait_" + trait + ".csv")
    save_csv(Tab_ok, dir_outs)

    Hyper_df_ok = pd.DataFrame(BO.max)
    Hyper_df_ok["Parameter"], Hyper_df_ok["Fold"] = Hyper_df_ok.index, ok
    dir_outs = os.path.join(dir_outs0, "Hypers_" + predictor_label + "-Trait_" + trait + ".csv")
