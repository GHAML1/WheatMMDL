...
dir_outs0 = os.path.join(dir, "dataset_" + datasets[datanumber].replace("-PP", ""),
                         "Model_v8_v6", "CV_by_year", "trait_" + trait)
os.makedirs(dir_outs0, exist_ok=True)

dir_folds = os.path.normpath(os.path.join(
    dir_outs0, "../../../folds_df_dataset_" + datasets[datanumber] + "-by_year.csv"
))
...
for year in years:
    ...
    for ok in range(1, K + 1):
        ...
        dir_outs = os.path.join(dir_outs0, prefix + "-Preds_" + predictor
                                + "-Trait_" + trait + ".csv")
        save_csv(df_Preds, dir_outs)
        dir_outs = os.path.join(dir_outs0, prefix + "-Tab-Smm_" + predictor
                                + "-Trait_" + trait + ".csv")
        save_csv(Tab_ok, dir_outs)

        Hyper_df_ok = pd.DataFrame(BO.max)
        Hyper_df_ok["Parameter"], Hyper_df_ok["Fold"] = Hyper_df_ok.index, ok
        dir_outs = os.path.join(dir_outs0, prefix + "-Hypers_" + predictor
                                + "-Trait_" + trait + ".csv")
