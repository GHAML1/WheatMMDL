...
traits = c("BLUE_bbch59_gdd","BLUE_bbch59_gdd","BLUE_bbch59_gdd",
           "BLUE_bbch30_gdd")
traits_covs = list(c("BLUE_bbch10_gdd","BLUE_bbch30_gdd"),"BLUE_bbch30_gdd",
                   "BLUE_bbch10_gdd","BLUE_bbch10_gdd")
traits = sub('BLUE_','',traits)
traits
traits_covs = sapply(traits_covs, function(x)sub('BLUE_','',x))
vars = c('NIRs', 'Drone')#'Weather_Smms'

## Fixed example
year <- years[1]
trait <- traits[1]#"bbch59_gdd"
trait_covs <- traits_covs[1]#c("bbch10_gdd", "bbch30_gdd")
predictor <- "GID+NIRs"
...
ETA_t <- ETA[c("GID", "NIRs")]

# Fixed covariates
ETA_t$Traits_covs <- list(
  X = pheno_r[, trait_covs, drop = FALSE],
  model = "FIXED"
)
